...
- **Pages**:
  ...
  - `ElencoAttivitaPage` — Activity list with filters, grouped view, detail drawer, CSV export; due viste: "Standard" e "Ordini bucket" (tipo `BUCKET`, con righe espandibili sul rapportino mensile consuntivate/fatturate). La vista bucket è **sempre raggruppata per cliente** (il toggle "Raggruppa per Progetto" è nascosto: un ordine bucket ha 0..N progetti indicativi) e il KPI "Progetti" diventa "Clienti"
  ...
  - `ContrattiPage` (prefisso CSS `ct-`) — contratti di assistenza/AMS per cliente (sostituisce l'Excel "Contratti progetti"), visibile a Board/PM/Account: banner warning contratti in scadenza (dataFine entro 60 giorni, stati non `isChiuso`), filtri (anno di competenza esplicito, stato, tipo, cliente, PM), tabella raggruppata per cliente con righe espandibili, modal CRUD con applicazioni coperte (toggle-chips sui Progetti del cliente) e link Drive (`DriveLinkField` su radice Contratti). **Il PM non è un campo del contratto**: si eredita in sola lettura dai `pmRiferimento` dei progetti coperti (uno solo se un PM li gestisce tutti). Fatturazione = flag booleano `fatturato` (badge Fatturato/Da fatturare), nessun importo parziale. **Il contratto ha il SUO `riferimentoOrdineVendita`** (GO-ORDV) come le attività: l'import consuntivi Zoho aggancia le ore al contratto per corrispondenza di codice — nessun collegamento manuale ad attività. Confronto economico: consumato € = `giornateConsuntivate` (dal solo import Zoho) × `costo_medio_giornata` (AppConfig), barra vs `importoTotale` (niente barra se manca importo, costo medio o consuntivato). Azione "Clona su un altro anno" per riga (rinnovo annuale): date shiftate, titolo con anno aggiornato, stato/fatturato/consuntivato/ordine/Drive/note reset, applicazioni e importo copiati. (I campi `rinnovoTacito` e `disdettaEntro` sono stati rimossi)
...
- **REST API**: all routes protected by JWT middleware
  ...
  **Current routes (`/api/` prefix):**
  ...
  - `POST /api/attivita` — Create activity. `progettoId` è obbligatorio solo per le `STANDARD`: un ordine `BUCKET` vale a livello cliente e riceve invece `progettiIds: string[]` (0..N, indicativi)
  - `PUT /api/attivita/:id` — Update activity. Su un bucket, `progettiIds` assente = lista invariata, presente = sostituita per intero (azzerata d'ufficio se cambia il cliente)
  ...
  - **Zoho Projects — import consuntivazioni** (API **v3**, `https://projectsapi.zoho.eu/api/v3/portal/{portalId}/…`; tutte con middleware `requireRole('BOARD', 'PM', 'ACCOUNT')`, primo enforcement server-side dei ruoli; rispondono `503` se le env `ZOHO_*` mancano):
    ...
    - `POST /api/zoho/consuntivi/:projectId` — ore consuntivate di UN progetto aggregate per codice `GO-ORDV-YYYY-N` (join timelog → task → tasklist → milestone/phase, finestre di 6 mesi paginate a 200 record — vedi `services/zohoService.ts`); il frontend itera sui progetti selezionati e somma i codici (limiti subrequest Workers + rate limit Zoho 200 req/2min **per endpoint**)
    - `POST /api/zoho/import/preview` — diff codici aggregati vs attività **e contratti assistenza/AMS** (match su `riferimentoOrdineVendita` = codice senza prefisso `GO-ORDV-`, come l'import CSV manuale; sui contratti il prefisso è tollerato). Risposta: `{matched, matchedContratti, notFound}` — un codice finisce in notFound solo se non corrisponde né ad attività né a contratti
    - `POST /api/zoho/import/confirm` — conferma dell'import: body `{updates, contratti}`; applica gli aggiornamenti alle attività (stessa semantica di `bulk-consuntivato`, valori "prima" riletti dal DB, breakdown mensile in `AttivitaConsuntivoMese` preservando le fatturate del PM) e ai contratti (`Contratto.giornateConsuntivate`, solo totale) e registra una `ZohoImportSession` con i delta (righe contratto: `contrattoId` + `progetto: "Contratto assistenza/AMS"`, stessa forma delle righe attività)
  ...
  - **Contratti assistenza/AMS** (tutte con `requireRole('BOARD', 'PM', 'ACCOUNT')`):
    - `GET /api/contratti` — lista con cliente e applicazioni (progetti, incluso `pmRiferimento`); `?anno=` filtra per anno di competenza
    - `POST /api/contratti` / `PUT /api/contratti/:id` — payload include `applicazioniIds` (join sostituita a ogni PUT); `giornateConsuntivate` NON è scrivibile da qui (arriva solo dall'import Zoho)
    - `POST /api/contratti/:id/clona` — body `{anno}`: clona su un altro anno di competenza (date shiftate della differenza di anni, titolo con anno sostituito, stato reset a IN_DEFINIZIONE, fatturato/consuntivato/ordine di vendita/Drive/note azzerati, applicazioni e importo copiati)
    - `DELETE /api/contratti/:id`
    - `GET /api/stati-contratto` (autenticati) + `POST/PUT/DELETE /api/stati-contratto/:id` (solo BOARD) — stati configurabili con flag `isChiuso` (fuori dal banner scadenze)
    - `GET /api/config/contratti` (Board/PM/Account) / `PUT` (solo BOARD) — `costoMedioGiornata` (AppConfig `costo_medio_giornata`, vuoto = confronto economico disattivato)
    - la config Google Drive (`/api/config/google-drive`) include la terza radice `contrattiUrl`/`contrattiId` (AppConfig `gdrive_contratti_*`, shared drive "Contratti annuali clienti e prodotti"); il PUT richiede tutte e tre le stringhe
...
- **Environment variables** (via `.env` — gitignored): `DATABASE_URL`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `JWT_SECRET`, `FRONTEND_URL`, `BACKEND_URL`, `PORT`, più `ZOHO_CLIENT_ID`/`ZOHO_CLIENT_SECRET`/`ZOHO_REFRESH_TOKEN`/`ZOHO_PORTAL_ID` (+ opzionali `ZOHO_ACCOUNTS_URL`/`ZOHO_PROJECTS_API_URL`, default datacenter EU) per l'import consuntivazioni da Zoho Projects. Gli **scope OAuth** del refresh token devono includere `ZohoProjects.projects.READ`, `tasklists.READ`, `milestones.READ`, `timesheets.READ` e **`tasks.READ`** (l'ultimo serve alla v3 per risalire dal timelog alla milestone; gli scope si fissano alla generazione del token, non si aggiungono a posteriori). NB: `npm run dev` carica `.env` via dotenv-cli **all'avvio** — dopo una modifica a `.env` serve riavviare il dev server (nodemon non lo rilegge)
- **Local JWT secret**: `dev-local-secret-cambia-in-prod` (the value in local `.env` — different from what DEV_SETUP.md may say)

### Prisma Schema Key Models

- `User` — **unified people directory** (was split across `ProjectManager` + `Account` until the 2026-07 migration; those models no longer exist). Fields: `googleId`/`email`/`name` (all nullable — a user can exist purely as anagrafica, never having logged in), `firstName`, `lastName`, `avatarUrl`, `roles: UserRole[]` (Postgres array, default `[]`). `enum UserRole { ACCOUNT PM BOARD DEVHUB }` — **4 fixed application roles**, not a user-editable list; a user can hold multiple roles at once (e.g. `{PM, ACCOUNT}`). Relations: `progettiPO` (PO of a Progetto), `progettiDevHub` (Responsabile DevHub of a Progetto), `attivitaPM` (PM of an Attivita, via `Attivita.pmId` — **un solo PM per attività**), `clientiAccount` (Account of a Cliente), `attivitaAccount` (Account of an Attivita), plus the pre-existing OAuth/Gantt relations (`ownedProjects`, `memberships`, `assignedTasks`, etc.)
...
- `Attivita` — activity tracking: `cliente`, `progetto`, `attivita`, `stato` (string key → `StatoAttivitaConfig`), dates (`inizio`, `deadline`), `giornateVendute`, `giornateConsuntivate`, notes, `accountId` → `User` (role `ACCOUNT`), `pmId` → `User` (role `PM`) — **un solo PM per attività** (era `AttivitaPM` many-to-many fino al 2026-07, join table rimossa con migrazione data-preserving). **Copertura bucket**: `bucketId` (nullable, self-relation verso una riga `tipo: BUCKET` dello stesso cliente in stato APERTA, `onDelete: SetNull`) — l'attività STANDARD confermata dal presale può essere coperta da un ordine bucket invece che da un ordine dedicato (scelta a due vie nel modal "Conferma e avvia" e nel form di modifica). **Mutualmente esclusivo con `riferimentoOrdineVendita`** (bucket impostato ⇒ ordine forzato a null, server-side su POST/PUT/PATCH stato): i consuntivi Zoho si agganciano SOLO al codice del bucket, mai all'attività coperta (niente doppio conteggio; il consumato per singola attività coperta non è tracciato). UI: badge bucket nella colonna Ord. Vendita e nel drawer (vista Standard); nella vista Ordini bucket il bottone "Aggiungi" è a due vie (nuovo ordine bucket / attività standard coperta da un bucket) e la riga espansa elenca le "Attività coperte da questo bucket" (esposte da `GET /api/attivita?tipo=BUCKET` come `attivitaCollegate`, forma completa) con dettaglio al click, matita di modifica e azione "Aggiungi attività a questo bucket" (bucket+cliente preselezionati)
- `AttivitaProgetto` — join **ordine bucket ↔ progetti "indicativi"** (0..N), stessa forma di `ContrattoApplicazione` (chiave composta, Cascade su entrambi i lati, lista sostituita per intero a ogni PUT). Un ordine bucket è capienza **a livello cliente**: i consuntivi Zoho si agganciano al suo `riferimentoOrdineVendita` e la copertura delle attività standard è validata sul solo cliente, quindi i progetti collegati sono una pura etichetta descrittiva e **non entrano in nessuna aggregazione economica**. Sulle righe `STANDARD` resta invece `progettoId` (uno solo, obbligatorio). Conseguenze: sui BUCKET `progettoId` è `null` e `progetto` è `''` (il payload API espone `progetti: {id,nome}[]` e, come forma testuale per CSV, `progetto` = nomi separati da virgola); i progetti devono appartenere al cliente dell'ordine (400 altrimenti) e vengono azzerati se cambia il cliente; il responsabile DevHub si eredita in sola lettura come **insieme distinto** dei `responsabileDevHub` dei progetti collegati (`devHubs: string[]`, vuoto se non c'è alcun progetto — `devHub`/`devHubId` singoli restano valorizzati solo quando l'insieme ha un solo elemento). Migrazione data-preserving `20260727150000_attivita_bucket_progetti_multipli`: il progetto singolo dei bucket esistenti è travasato nella join e poi sganciato dalla colonna
...
- `Contratto` — contratto di assistenza/AMS di un `Cliente` (obbligatorio, `onDelete: Restrict`): `titolo`, `tipo` (enum `TipoContratto`: `MANUTENZIONE` | `MANUTENZIONE_AMS`), `anno` (competenza esplicita, come i fogli Excel annuali), `stato` (string key → `StatoContrattoConfig`), `dataInizio`/`dataFine` (fine null = continuativo; dataFine alimenta il banner scadenze — `rinnovoTacito` e `disdettaEntro` sono stati rimossi), `importoTotale` (riferimento del confronto economico — il campo budgetOrdini è stato rimosso), `fatturato` (flag booleano, niente importi parziali né rate), `riferimentoOrdineVendita` (l'ordine di vendita GO-ORDV **del contratto**, come le attività) + `giornateConsuntivate` (scritte SOLO dall'import Zoho per corrispondenza di codice), `driveUrl`/`driveFolderId`. **Nessun campo referente/PM**: il PM si eredita in sola lettura dai `pmRiferimento` dei progetti coperti. Relazioni: `applicazioni` (join `ContrattoApplicazione` → `Progetto`) — nessun collegamento ad attività
- `StatoContrattoConfig` — stati contratto configurabili (`chiave`, `label`, `colore`, `isChiuso`, `ordine`); seed: In via di definizione / OK, a posto / Con problemi / Chiuso
