...
- **Pages**:
  ...
  - `ContrattiPage` (prefisso CSS `ct-`) — contratti di assistenza/AMS per cliente (sostituisce l'Excel "Contratti progetti"), visibile a Board/PM/Account: banner warning contratti in scadenza (dataFine o disdettaEntro entro 60 giorni, stati non `isChiuso`), filtri (anno di competenza esplicito, stato, tipo, cliente, PM), tabella raggruppata per cliente con righe espandibili, modal CRUD con applicazioni coperte (toggle-chips sui Progetti del cliente) e link Drive (`DriveLinkField` su radice Contratti). **Il PM non è un campo del contratto**: si eredita in sola lettura dai `pmRiferimento` dei progetti coperti (uno solo se un PM li gestisce tutti). Fatturazione = flag booleano `fatturato` (badge Fatturato/Da fatturare), nessun importo parziale. **Il contratto ha il SUO `riferimentoOrdineVendita`** (GO-ORDV) come le attività: l'import consuntivi Zoho aggancia le ore al contratto per corrispondenza di codice — nessun collegamento manuale ad attività. Confronto economico: consumato € = `giornateConsuntivate` (dal solo import Zoho) × `costo_medio_giornata` (AppConfig), barra vs `importoTotale` (niente barra se manca importo, costo medio o consuntivato). Azione "Clona su un altro anno" per riga (rinnovo annuale): date shiftate, titolo con anno aggiornato, stato/fatturato/consuntivato/ordine/Drive/note reset, applicazioni e importo copiati. Il dettaglio espanso mostra la voce Rinnovo solo se tacito o con termine di disdetta (mai "esplicito")
...
- **REST API**: all routes protected by JWT middleware
  ...
  **Current routes (`/api/` prefix):**
  ...
  - **Contratti assistenza/AMS** (tutte con `requireRole('BOARD', 'PM', 'ACCOUNT')`):
    ...
    - `POST /api/contratti/:id/clona` — body `{anno}`: clona su un altro anno di competenza (date/disdetta shiftate della differenza di anni, titolo con anno sostituito, stato reset a IN_DEFINIZIONE, fatturato/consuntivato/ordine di vendita/Drive/note azzerati, applicazioni e importo copiati)
  ...
  - **Roadmap → Attività** (ciclo di vita prodotti): `POST /api/roadmap-items/:id/avvia` — crea un'`Attivita` collegata alla card (body: `clienteId?`, `giornateVendute?`, `riferimentoOrdineVendita?`, `pmId?`): con `clienteId` è la quota commissionata dal cliente (account ereditato dal cliente), senza è la quota di investimento interna (`cliente: "Interno"`, giornate lette come **stanziamento** — lo sforamento segnala l'investimento oltre il previsto). Ripetibile: una chiamata per committente (co-finanziamento). `GET /api/roadmap-items` espone per ogni card gli aggregati derivati dalle figlie: `attivitaCollegate`, `finanziamento` (`INVESTIMENTO`/`FINANZIATA`/`CO_FINANZIATA`/null), `clientiFinanziatori`, `totaleVendute`, `totaleConsuntivate` — mai persistiti sulla card
...
- `Attivita` — activity tracking: `cliente`, `progetto`, `attivita`, `stato` (string key → `StatoAttivitaConfig`), dates (`inizio`, `deadline`), `giornateVendute`, `giornateConsuntivate`, notes, `accountId` → `User` (role `ACCOUNT`), `pmId` → `User` (role `PM`) — **un solo PM per attività** (era `AttivitaPM` many-to-many fino al 2026-07, join table rimossa con migrazione data-preserving)
- `Attivita.roadmapItemId` (nullable, SetNull) — card roadmap da cui l'attività discende o a cui è agganciata: il finanziamento di una feature di prodotto si legge dalle figlie (`clienteId` = chi paga, null = quota interna "Interno"). La RoadmapPage ha l'azione "Avvia" su card e lista (modal con committente opzionale, giornate precompilate da stima − già coperte, ordine di vendita, PM), badge finanziamento + mini barra consuntivato/venduto sulla card, elenco figlie nei modal di modifica/dettaglio
